 ...
 *
 *
 * @method     ChildUsersQuery orderByUserPk($order = Criteria::ASC) Order by the user_pk column
 ...
 *
 * @method     ChildUsersQuery groupByUserPk() Group by the user_pk column
 ...
 *
 * @method     ChildUsersQuery leftJoinUserDossierMapping($relationAlias = null) Adds a LEFT JOIN clause to the query using the UserDossierMapping relation
 * @method     ChildUsersQuery rightJoinUserDossierMapping($relationAlias = null) Adds a RIGHT JOIN clause to the query using the UserDossierMapping relation
 * @method     ChildUsersQuery innerJoinUserDossierMapping($relationAlias = null) Adds a INNER JOIN clause to the query using the UserDossierMapping relation
 *
 * @method     ChildUsersQuery joinWithUserDossierMapping($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the UserDossierMapping relation
 *
 * @method     ChildUsersQuery leftJoinWithUserDossierMapping() Adds a LEFT JOIN clause and with to the query using the UserDossierMapping relation
 * @method     ChildUsersQuery rightJoinWithUserDossierMapping() Adds a RIGHT JOIN clause and with to the query using the UserDossierMapping relation
 * @method     ChildUsersQuery innerJoinWithUserDossierMapping() Adds a INNER JOIN clause and with to the query using the UserDossierMapping relation
 *
 * @method     \UserDossierMappingQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 *
 * @method     ChildUsers findOneByUserPk(int $user_pk) Return the first ChildUsers filtered by the user_pk column
 ...
 *
 * @method     ChildUsers requireOneByUserPk(int $user_pk) Return the first ChildUsers filtered by the user_pk column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildUsers[]|ObjectCollection findByUserPk(int $user_pk) Return ChildUsers objects filtered by the user_pk column
...
abstract class UsersQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'garantieapp', $modelName = '\\Users', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT user_pk, name, email, password FROM users WHERE user_pk = :p0';
        try {
    ...
    public function filterByPrimaryKey($key)
    {
        ...
        return $this->addUsingAlias(UsersTableMap::COL_USER_PK, $key, Criteria::EQUAL);
    }
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        return $this->addUsingAlias(UsersTableMap::COL_USER_PK, $keys, Criteria::IN);
    }
    ...
    public function filterByUserPk($userPk = null, $comparison = null)
    {
        if (is_array($userPk)) {
            $useMinMax = false;
            if (isset($userPk['min'])) {
                $this->addUsingAlias(UsersTableMap::COL_USER_PK, $userPk['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($userPk['max'])) {
                $this->addUsingAlias(UsersTableMap::COL_USER_PK, $userPk['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UsersTableMap::COL_USER_PK, $userPk, $comparison);
    }
    ...
    /**
     * Filter the query by a related \UserDossierMapping object
     *
     * @param \UserDossierMapping|ObjectCollection $userDossierMapping the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildUsersQuery The current query, for fluid interface
     */
    public function filterByUserDossierMapping($userDossierMapping, $comparison = null)
    {
        if ($userDossierMapping instanceof \UserDossierMapping) {
            return $this
                ->addUsingAlias(UsersTableMap::COL_USER_PK, $userDossierMapping->getUserFk(), $comparison);
        } elseif ($userDossierMapping instanceof ObjectCollection) {
            return $this
                ->useUserDossierMappingQuery()
                ->filterByPrimaryKeys($userDossierMapping->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByUserDossierMapping() only accepts arguments of type \UserDossierMapping or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the UserDossierMapping relation
     *
     ...
     * @return $this|ChildUsersQuery The current query, for fluid interface
     */
    public function joinUserDossierMapping($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('UserDossierMapping');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'UserDossierMapping');
        }
    ...
    /**
     * Use the UserDossierMapping relation UserDossierMapping object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \UserDossierMappingQuery A secondary query class using the current class as primary query
     */
    public function useUserDossierMappingQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinUserDossierMapping($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'UserDossierMapping', '\UserDossierMappingQuery');
    }
    ...
    {
        if ($users) {
            $this->addUsingAlias(UsersTableMap::COL_USER_PK, $users->getUserPk(), Criteria::NOT_EQUAL);
        }
