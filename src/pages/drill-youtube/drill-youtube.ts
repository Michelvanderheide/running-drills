...
export class DrillYoutubePage {
  ...
  constructor(nav: NavController, navParams: NavParams, drillData:DrillData, public sanitizer: DomSanitizer, private file:File) {
    this.nav = nav;
    ...
    */
            ...
            this.file.checkFile(cordova.file.applicationDirectory, 'www/build/video/'+this.videoFile).then(
              (fileExists: boolean) => {
                  if (fileExists) {
